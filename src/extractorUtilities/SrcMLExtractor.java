...
public class SrcMLExtractor implements ExtractionMethod {
	...
	public SrcMLExtractor(String target, String rootPath, String projectPath) {
		this.target = target;
	...
	public boolean doExtraction(String language) {
		...
		String list_of_files_path = DynamicPaths.getPath() + "list_of_files_src_ml";
...
//				int i = 0;
			...
			while (br.ready()) {
				...
				if (line.endsWith(".c") || line.toLowerCase().endsWith(".cc") || line.toLowerCase().endsWith(".cpp") || 
						...
						line.toLowerCase().endsWith(".java") || line.toLowerCase().endsWith(".cxx") || 
						line.toLowerCase().endsWith(".hxx")) { 
...
//			String[] ccccCommand = new String[] { "/bin/bash", "-c", "./run_multimetric.sh" };
			...
			System.out.println("Error while opening the list of files in SrcMLExtractor: \nError Message \n"
...
//	}
	...
	public void storeExtraction() {
		// this.rootPath;
		String PathToStore = DynamicPaths.getPath() + "srcml/" + this.projectName + "/";
		File storage = new File(PathToStore);
		if (!storage.exists())
			storage.mkdirs();
		String[] ccccCommand = {"cp",  "/vagrant/srcml_output.xml", PathVMRectifier.rectify(PathToStore) + this.target + ".xml"};
		System.out.println(ConsoleFactory.getConsole().run(ccccCommand, null, null));
	}
