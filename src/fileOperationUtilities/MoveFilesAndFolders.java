...
public class MoveFilesAndFolders{
	...
	/**
	 * move filename from the rootPath to the destinationPath provided at instantiation of the Class doesn't work if files are within any
	 * folder depth. If your files are nested in folder use moveInDepth()
	 * 
	 * @param fileName
	 */
	public void move(String fileName){
		...
		ConsoleFactory.getConsole().run("cp -a " + source + " " + target);
	}
	...
	/**
	 * move any file from rootPath to destinationPath and create all necessary folders to mimic the exact folder structure of the sourcePath
	 * and filename
	 * 
	 * @param fileName
	 */
	public void moveInDepth(String fileName){
		...
		while (i < sourceParts.length - 1) {
			...
			i++;
			if (Files.notExists(Paths.get(sourcePathIncremental)))
	...
	public void moveAll() {
		...
		ConsoleFactory.getConsole().run("cp -R " + this.rootPath.substring(0, this.rootPath.length()-1) + " " + this.destinationPath);
	}

	public void deleteDestinationFolder(){
		ConsoleFactory.getConsole().run("rm " + this.destinationPath);
	}
	...
	private void mkdir(String directoryPath){
		ConsoleFactory.getConsole().run("mkdir " + directoryPath);
	}
